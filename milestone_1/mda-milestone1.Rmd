...
<!-------------------------- Start your work below ---------------------------->
**Task 1.2 Answer**
**Dataset 1: Examine cancer_sample dataset**
print out three attributes:
...
```
**Dataset 2: Examine steam_games dataset**
print out three attributes:
...
glimpse(steam_games)
```
**Dataset 3: Examine apt_buildings dataset**
print out three attributes:
...
**Dataset 4: Examine vancouver_trees dataset**
print out three attributes:
...
<!-------------------------- Start your work below ---------------------------->
**Task 1.3 Answer:** I would like to choose cancer_dataset. This dataset provides a wide range of measurements describing the features of benign and malignant cancer cell nuclei,
I'm curious to find out how those cell features may correlate and is more deterministic towards diagnosis of cancer cells.  Ref: [Breast Cancer Wisconsin (Diagnostic) Dataset](https://archive.ics.uci.edu/dataset/17/breast+cancer+wisconsin+diagnostic)
...
**Task 1.4 Answer:** My first research question is whether the radius of the cell nuclei correlates to the diagnosis of the cell healthiness and if so, how do they correlate?
<!----------------------------------------------------------------------------->
...
<!-------------------------- Start your work below ---------------------------->
**Task 2.1, 2.2 Answer:** I picked exerciese **1**, **2**, **6**, **8** to attempt.

**Exercises 1**: Plot the distribution of radius_mean

**Explanation**: I have a sense of feeling that mean radius of the cell nuclei is related to whether the cell is a benign cell or malignant one. Therefore, I'm intrigued to find out the distibution of radius_mean as a key feature.

```{r exercise1-mean-radius-histogram, fig.cap="Mean Radius Histogram"}
...
**Exercises 2**: Create a nuclei_size_index to represent the overall size based on perimeter_mean, radius_mean abd area_mean.

**Explanation**: I want to create this new variable to indicate the size of cell nuclei because I feel radius, perimeter and area are all somehow related to the overall size. Creating this new variable may link all those three variables together to be an indicator for malignant or benign cells.

```{r exercise2-new-variable-histogram, fig.cap="Mean Size Index Histogram"}
...
**Exercise 6**: Use a boxplot to look at the frequency of different observations within texture_worst.

**Explanation**: I have speculations that texture may also be an important factor for diagnsis of cancer. However, just looking at mean texture may not be so obvious, therefore, I chose texture_worst to see if this is feature is related to cancer diagnosis.


```{r exercise6-texture-worst-boxplot, fig.cap="Texture Worst Boxplot"}
...
**Explanation**: As I have already examined the mean radius, I want to also see the exetreme cases: whether worst radius is related to the diagnosis of cancer. Therefore, I chose exercies 8 to make density plot.

```{r exercise8-radius-worst-density-plot, fig.cap="Radius Worst Density Plot"}
...
print(density_plot)
```
Observation: from the plot, I found that worst radius could be an important factor for predicting cancer as cancer cells have greater worst radius. However, just using this as a prediction feature is not enough and may lead to false prediction.
...
1. Are the size of cell nuclei related to the smoothness or other numerical features? If so, how do they correlate and how will they help predict cancer diagnosis?

2. From the dataset, each feature (such as radius, texture, concavity etc) are provided with mean, worst, and standard error measurements to determine the characteristics of tumors. For each measurement of the features, which is more predictive, (the mean, worst or SE)?

3. Among all the features, which ones are the most dominant features for predicting tumors? How can we find out?

4. Is it possible to use some ML algorithms such as random forest, XGboost to perform cancer diagnosis on this dataset?
